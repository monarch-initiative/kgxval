...
def makePipeJoinedStringIfList(str_or_list: str | list[str]) -> str:
    ...
    raise ValueError(f"Can't pipe join of \"f{str_or_list}\". Type is {type(str_or_list)}.")
...
class SPQOStats:
    ...
    def _updatePubCount(self, edge: KGX_EDGE):
        if "publications" in edge and edge["publications"] is not None:
            pub_cnt = len(edge.get("publications",[]))
    ...
    def _updateAKS(self, edge: KGX_EDGE):
        ...
        elif "aggregator_knowledge_source" in edge and edge["aggregator_knowledge_source"] is not None:
            self.aks_set.add(makePipeJoinedStringIfList(edge["aggregator_knowledge_source"]))
    ...
    def makePdOutputRow(
        self, kgx_infores: str, normalized: str, total_edge_cnt: int
    ) -> PD_SUMM_ROW:
...
class KGXSummarizer(Ingest):
    ...
    def initWithIngestObj(
        cls, ingest_obj: Ingest, high_priority_node_cats: Iterable[str]
    ):
