...
def findEdgeFile(ingest_dir, find_normalized=True) -> Optional[str]:
    for (root,_,files) in os.walk(ingest_dir):
        for filename in files:
            if(not find_normalized):
                if(filename.endswith("edges.jsonl") and not filename.endswith("normalized_edges.jsonl") and not filename.endswith("example_edges.jsonl")):
                    return os.path.join(root,filename)
            if(find_normalized):
...
class Ingest:
    def __init__(self,ingest_name:str,node_path:Path,edge_path:Optional[Path],norm_status:str=""):
