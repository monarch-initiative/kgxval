...
def getNodeStructuralErrors(ingest: Ingest, fail_on_invalid: bool) -> list[PREFIX_ERR]:
    ...
    for node_dict in ingest.iter_nodes():
        if ":" not in node_dict["id"]:
            _printError(
                f"Invalid Node ID found in {ingest.node_path.name} --- {node_dict["id"]} --- {node_dict}",
                fail_on_invalid,
            )
            errors.append(
                PREFIX_ERR(
...
def _checkValidPrefixForClass(id_prefix: str, blink_class: str):
    ...
    if blink_class not in blink_class_to_idprefix:
        return True  # No range constraint to violate
    elif len(blink_class_to_idprefix[blink_class]) == 0:
        return True  # No range constraint to violate
    else:
...
def validateNodePrefixesForIngest(
    ingest: Ingest, fail_on_invalid: bool = False
) -> list[PREFIX_ERR]:
    errors: list[PREFIX_ERR] = list()
...
def writeErrorsToFile(error_dict: dict[tuple[str, str], set[str]], output_path: Path):
    ...
    with open(output_path, "w") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["INVALID_ID_PREFIX", "BIOLINK_CLASS", "BIOLINK_URL", "SOURCES_APPEARS_IN"]
        )
        for (id_prefix, category), sources in error_dict.items():
            ...
            writer.writerow(
                [id_prefix, category, blink_url, ", ".join(sorted(sources))]
            )
...
def main():
    ...
    if len(ingest_dicts) < 0:
        raise ValueError(
            f"Could not find any ingests at $INGEST_TOP_LEVEL_DIR --- {ingest_dir}"
        )
    for source_name in ingest_dicts:
        if "not_normalized" in ingest_dicts[source_name]:
            l = validateNodePrefixesForIngest(
                ingest_dicts[source_name]["not_normalized"]
            )
            print(l)
