...
from kgxval.dir.utils import JSONLDictGen, TSVDictGen
...
#XKCD CHANGE FOR ITRB
#    norm_node_path = findNodeFile(source_dir, find_normalized=False)
#    norm_edge_path = findEdgeFile(source_dir, find_normalized=False)
...
def makeIngestObjsDict(top_level_dir_path: Path | str) -> INGEST_MAP:
    ...
    #This is a correction because ITRB releases only "MERGED" data (which is normalized).
    # No other data is included and all files are in format "{infores}/nodes.jsonl" and "{infores}/edges.jsonl" 
    if(all("normalized" not in ingest_dict[x] for x in ingest_dict)):
        ingest_dict2: dict[str, dict[str, Ingest]] = defaultdict(dict)
        for source_name in ingest_dict:
            #ingest_dict2[source_name]["merged"] = ingest_dict[source_name]["not_normalized"]
            ingest_dict2[source_name]["normalized"] = ingest_dict[source_name]["not_normalized"]
        return ingest_dict2

    return ingest_dict

def iter_all_edges(ingest_map: INGEST_MAP, norm_status: str, ignore_translator_kgs:bool=False) -> Iterable[tuple[dict[str, Any],Ingest]]:
    for infores in ingest_map:
        #Skip existing aggregate kgx files; translator_kg and translator_kg_open 
        if(ignore_translator_kgs):
            if("translator_kg" in infores):continue
        
        if(norm_status not in ingest_map[infores]): continue
        ingest = ingest_map[infores][norm_status]
        for edge in ingest_map[infores][norm_status].iter_edges():
            yield(edge,ingest)