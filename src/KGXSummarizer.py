...
class SPQOStats:
    ...
    def _getPropertyKeys(self,edge:dict) -> set[str]:
        ...
        for potential_prop,val in edge.items():
            ...
            if(potential_prop in self.prop_set):continue #We've already seen this property, no reason to check if it's populated. 
    ...
    def incrementStats(self,edge):
        ...
        if(len(at_str)>0): self.at_set.add(at_str)
...
class KGXSummarizer(Ingest):
    ...
    def __init__(self,ingest_name:str,node_path:Path,edge_path:Optional[Path],norm_status:str,high_priority_node_cats:Iterable[str]):
        self.tk = Toolkit()
        ...
        for blink_class in self.tk.get_all_classes():
            self.blink_class_to_depth[blink_class] = self.tk.get_element_depth(blink_class)
        super().__init__(ingest_name=ingest_name,node_path=node_path,edge_path=edge_path,norm_status=norm_status)

    @classmethod
    def initWithIngestObj(cls,ingest_obj:Ingest,high_priority_node_cats:Iterable[str]):
        ...
        return cls(ingest_name=ingest_name,
                      node_path=node_path,
                      edge_path=edge_path,
    ...
    def _getBestCat(self,category_list:Collection[str]) -> str:
        ...
        for (hp_node_cat,desc_set) in self.high_priority_desc_dict.items():
            if(len(desc_set.intersection(category_list))>0):return hp_node_cat
    ...
    def summarize_edges(self) -> list[dict]:
        ...
        for edge_dict in self.iter_edges():
            ...
            self.spqo_to_stats[edge_spqo].incrementStats(edge_dict)
