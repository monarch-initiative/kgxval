...
def JSONLDictGen(file_loc: Optional[Path],attach_original_json:bool=False) -> Iterable[KGX_NODE | KGX_EDGE]:
    if(file_loc is None):
        return
...
def TSVDictGen(file_loc: Optional[Path],_:bool=False) -> Iterable[KGX_NODE | KGX_EDGE]:
    if(file_loc is None):
        return
