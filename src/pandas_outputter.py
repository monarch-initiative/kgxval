...
def makeSampleDF(kgx_samples:list[dict]):
    ...
    known_cols = ["KGX Infores", "SPQO Tuple", "id", "category", "subject", "sub name", "predicate", "object", "obj name", "qualified_predicate"] \
        + orderQualifiersForMatt(output_quals) + output_roles + ["original_subject", "original_object", 'original_json']
    ...
    final_col_list = ["KGX Infores", "SPQO Tuple", "id", "category", "subject", "sub name", "predicate", "object", "obj name", "qualified_predicate"] \
        ...
        + other_keys + ["original_subject", "original_object", "original_json"]
...
#    unnorm_df,unnorm_samples_df,norm_df,norm_samples_df = None,None,None,None
    ...
    if(unnorm_exists):
        ...
        if(config.unnorm_summ): 
            ...
            unnorm_df.to_excel(writer,sheet_name=f"{source_name}_unnorm_summary",index=False)
        if(config.unnorm_samp):
            updatePBar("unnorm sampling")
            unnorm_samples_df = makeSampleDF(unnorm_summ.sample_edges())
            unnorm_samples_df.to_excel(writer,sheet_name=f"{source_name}_unnorm_samples",index=False)
    ...
    if(norm_exists):
        ...
        if(config.norm_summ):
            ...
            norm_df.to_excel(writer,sheet_name=f"{source_name}_normalized_summary",index=False)
        if(config.norm_summ):
    ...
    if(norm_exists and config.include_rollup):
        ...
        rollup_summ.summarize_edges()
        rollup_df = makeSummaryDF(rollup_summ.get_pd_rows(),rollup=True)
        rollup_df.to_excel(writer,sheet_name=f"{source_name}_rollup_summary",index=False)
        updatePBar("rollup sampling")
        rollup_samples_df = makeSampleDF(rollup_summ.sample_edges())
        rollup_samples_df.to_excel(writer,sheet_name=f"{source_name}_rollup_samples",index=False)
