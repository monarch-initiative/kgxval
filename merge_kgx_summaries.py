...
def getNorm(xlsx_file_path):
    ...
    assert len([x for x in ef.sheet_names if "normalized_summary" in x])==1, f"BAD, {xlsx_file}"
...
writer = pd.ExcelWriter(os.path.join("data/combined_output/",f"{latest_date}_combined.xlsx"))
